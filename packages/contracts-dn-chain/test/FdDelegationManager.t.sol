...
contract FdDelegationManagerTest is Test {
    ...
    address public governance = address(0x09);
    ...
    function testOperatorCanRegisterAndEmitEvents() public {
        ...
        vm.prank(operator);
        fdChainDepositManager.depositIntoDolphinnetChain{value: 1 ether}(1 ether);
        ...
        vm.prank(operator1);
        fdChainDepositManager.depositIntoDolphinnetChain{value: 2 ether}(2 ether);
        vm.prank(operator1);
        delegationManager.registerAsOperator(od1, nodeUrl);
    ...
    function testStakerCanDelegateToOperator() public {
        ...
        assertEq(delegationManager.getOperatorShares(operator), 3 ether);
        assertEq(
    ...
    function testStakerCanUndelegate() public {
        ...
        assertEq(fdChainDepositManager.getDeposits(user1), 0);
    ...
    function testIncreaseAndDecreaseShares() public {
        ...
        vm.prank(user1);
        vm.expectRevert("onlyDepositManager");
        delegationManager.increaseDelegatedShares(user1, 100 ether);

        vm.prank(address(fdChainDepositManager));
        delegationManager.increaseDelegatedShares(user1, 100 ether);
        assertEq(delegationManager.getOperatorShares(operator), 103 ether);
        assertEq(
            delegationManager.stakerDelegateSharesToOperator(operator, user1),
            102 ether
        );
        ...
        vm.prank(user1);
        vm.expectRevert("onlyDepositManager");
        delegationManager.decreaseDelegatedShares(user1, 1 ether);
    ...
    function testSlashingStakingShares() public {
        ...
        delegationManager.slashingStakingShares(operator, user1, 1 ether);
    ...
    function testOperatorSharesIncreaseViaDelegate() public {
        _registerAndDelegate();
        assertGt(delegationManager.getOperatorShares(operator), 0); // 你可以 mock fdChainDepositManager 返回值
    }
